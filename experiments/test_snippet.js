(async function(){
  ...
  while (container && getComputedStyle(container).overflowY.match(/(auto|scroll)/) === null) {
  ...
  while (true) {
    const ticketsNow = Array.from(document.querySelectorAll('div.index_chat_4fr82'));
    const last = ticketsNow[ticketsNow.length - 1];
    const raw = last.querySelector('span.index_chatDate_z4mNc')?.textContent.trim();
    if (!raw) break;
  ...
  for (let el of document.querySelectorAll('div.index_chat_4fr82')) {
    const raw = el.querySelector('span.index_chatDate_z4mNc')?.textContent.trim() || '';
    const { day, month } = parseDM(raw);
    ...
    const num     = el.querySelector('span.index_chatConversationId_jfC7p')?.textContent.trim() || '';
    const msgDate = document.querySelector('.om_1_g0')?.textContent.trim() || '';
    const msgText = document.querySelector('.om_31_n6')?.textContent.trim() || '';
  ...
  console.table(rows);
  const header = ['Дата списка','№ обращения','Дата сообщения','Текст сообщения'];
  const csv = [
    header.join(','),
